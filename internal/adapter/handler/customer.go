...
func (p *Handler) CreateCustomer(ctx *gin.Context) {
	...
	result, err := p.svc.CreateCustomer(ctx, req)
	if err != nil {
...
func (h *Handler) ListProviderCustomer(ctx *gin.Context) {
	...
	result, count, err := h.svc.ListCustomer(ctx, &req)
	if err != nil {
		ErrorResponse(ctx, http.StatusInternalServerError, err)
		return
	}
...
func (h *Handler) GetCustomer(ctx *gin.Context) {
	id := ctx.Param("id")
	result, err := h.svc.GetCustomer(ctx, id)
	if err != nil {
		ErrorResponse(ctx, http.StatusInternalServerError, err)
		return
	}
...
func (h *Handler) UpdateCustomer(ctx *gin.Context) {
	...
	_, err := h.svc.UpdateCustomer(ctx, id, req)
	if err != nil {
		ErrorResponse(ctx, http.StatusInternalServerError, err)
		return
	}
...
func (ch *Handler) DeleteCustomer(ctx *gin.Context) {
	...
	result, err := ch.svc.GetCustomer(ctx, id)
	if err != nil {
		ErrorResponse(ctx, http.StatusInternalServerError, err)
		return
	}
	err = ch.svc.DeleteCustomer(ctx, id)
	if err != nil {
		ErrorResponse(ctx, http.StatusInternalServerError, err)
		return
	}
